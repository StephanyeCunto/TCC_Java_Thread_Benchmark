...
\begin{document}
    ...
    Este trabalho pode ser resumido nas seguintes etapas:


    \begin{enumerate}
        \item Projeto do \textit{benchmark}: desenvolvimento de um \textit{benchmark} com foco em operações de I/O.
        \item Criação do servidor: implementação de um servidor que permita a utilização de \textit{threads} tradicionais e virtuais.
        \item Experimento de carga constante: uma máquina cliente envia requisições ao servidor a uma taxa fixa.    
        \item Experimento de aumento gradual de carga: semelhante ao experimento de carga constante, porém com incremento progressivo da carga até o limite do servidor.
    \end{enumerate}
    ...
    Também foi utilizada uma máquina cliente, com as seguintes especificações:
    ...
    \section{Testes}
    Foram realizados dois tipos de testes: teste de carga constante e teste de aumento gradual de carga. Ambos tiveram como objetivo comparar o desempenho entre \textit{threads} tradicionais e \textit{threads} virtuais. Para garantir condições equivalentes entre as execuções, foram utilizados scripts automatizados que padronizaram tempos de aquecimento, pausas e coleta de lixo. A ferramenta Vegeta foi empregada tanto para o envio das requisições quanto para o monitoramento.

    Os testes foram executados dez vezes para cada mecanismo de \textit{threads}, totalizando vinte execuções. Esse número de repetições mostrou-se suficiente para garantir estabilidade estatística nos resultados obtidos.
    ...
    Os testes foram realizados em uma rede local e são descritos nas subseções seguintes.


    \subsection {Teste de Carga Constante}

    Para investigar a existência de diferenças entre os mecanismos de \textit{threads} ao longo de um período prolongado, foram realizados testes com carga constante. Para isso, fixou-se uma taxa de requisições baseada nos limites impostos pelo sistema operacional da máquina servidora.

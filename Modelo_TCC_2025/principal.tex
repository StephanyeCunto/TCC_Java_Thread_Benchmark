...
\begin{document}
    ...
    \subsection{Configurações de Sistema e Rede}
    ...
    \begin{itemize}
        \item \texttt{/threads/traditional}: Inicia uma thread tradicional e espera 100 ms antes de responder à requisição.
        \item \texttt{/threads/virtual}: Inicia uma thread virtual e espera 100 ms antes de responder à requisição.
        \item \texttt{/threads/gc}: Executa manualmente o processo de garbage collection.
    \end{itemize}
    ...
    O servidor foi configurado para registrar as seguintes métricas de desempenho utilização média da CPU, memória RAM e heap da JVM utilizando o Java Flight Recorder (JFR). Os dados foram salvos no arquivo Monitor.jfr e processados utilizando o Java Mission Control (JMC) para geração de tabelas.
    ...
    \section{Testes}
    Foram realizados dois tipos de testes: teste de carga constante e teste de aumento gradual de carga (\textit{ramp up}). Ambos tiveram como objetivo comparar o desempenho entre \textit{threads} tradicionais e \textit{threads} virtuais.

    Para garantir condições equivalentes entre as execuções, foram utilizados scripts automatizados que padronizaram tempos de aquecimento, pausas e coleta de lixo.Além disso, foi definida uma ordem de execução que alternava entre o uso de \textit{threads} tradicionais e virtuais, com o objetivo de minimizar possíveis vieses relacionados à ordem dos testes. A ferramenta Vegeta foi empregada tanto para o envio das requisições quanto para o monitoramento.

    Os testes foram executados quarenta e cinco vezes para cada mecanismo de \textit{threads}, totalizando noventa execuções. Esse número de repetições mostrou-se suficiente para garantir estabilidade estatística nos resultados obtidos.
    ...
    Os scripts utilizados nos experimentos e o código-fonte do servidor, estão disponíveis no repositório do GitHub: \url{https://github.com/StephanyeCunto/TCC_Java_Thread_Benchmark}.
    ...
    \end{enumerate}
     ...
     Cada execução teve duração aproximada de 17,5 minutos, considerando os tempos de aquecimento, pausas e coleta de lixo. Dessa forma, a execução completa dos testes com ambos os mecanismos de \textit{threads} totalizou aproximadamente 35 minutos. O tempo total de execução dos testes foi de cerca de 27 horas.

    \subsection {Teste de carga com aumento gradual (\textit{ramp up})}
    ...
    Dessa forma, a execução completa dos testes contemplando ambos os mecanismos de \textit{threads} totalizou aproximadamente 3 horas e 27 minutos por repetição. No total, o conjunto de experimentos demandou cerca de 155 horas e 15 minutos de execução contínua.

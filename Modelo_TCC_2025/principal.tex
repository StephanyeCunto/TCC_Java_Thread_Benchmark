...
\begin{document}
    ...
    Pode-se resumir este trabalho da seguinte maneira:

    \begin{enumerate}
        \item Projeto do benchmark: desenvolver um benchmark focado em I/O.
        \item Criação do servidor: criar um servidor que permita a utilização de threads tradicionais e virtuais.
        \item Experimento de carga constante: uma máquina cliente envia requisições ao servidor a uma taxa constante.
        \item Experimento de aumento gradual de carga: semelhante ao experimento de carga constante, mas com uma carga que aumenta até o limite do servidor.
    \end{enumerate}

    Todos os testes foram baseados nos testes realizados por \citeonline{GustafssonPersson2024}, adaptados para o contexto deste trabalho.

    Nas subseções seguintes, o método será detalhado.


    \section{Configuração de hardware}

    Os experimentos foram conduzidos em uma máquina que atuou como servidor, com as seguintes especificações:

    \begin{itemize}
        \item Processador: Apple M2
    ...
    Foi também utilizada uma máquina cliente com as seguintes especificações:

    \begin{itemize}
        \item Processador: Intel Core i5-2410M 
        \item Memória RAM: 8~GB
        \item Sistema Operacional: Ubuntu 22.04 LTS
        \item Vegeta: versão 12.8.4
    \end{itemize}

    \section{Métricas coletadas}

    O servidor foi configurado para registrar métricas de desempenho utilizando um script. As métricas coletadas incluem: utilização da CPU, número de threads, memória RAM e heap.

    A máquina cliente coletou as seguintes métricas: latência (mínima, máxima, média e percentis 50, 90, 95 e 99), taxa de transferência, taxa de amostragem, respostas do servidor (para cálculo da taxa de sucesso), bytes de entrada e bytes de saída. Esse monitoramento foi realizado utilizando a ferramenta Vegeta.

    Todos os dados são concatenados em arquivos CSV para análise.
    ...
    Como exemplo, alguns erros observados incluem:
    \begin{itemize}
    ...
    Esses registros permitiram monitorar a estabilidade do servidor e avaliar o limite máximo de requisições processáveis.

    \section{Testes}
    Dois tipos de testes foram realizados: teste de carga constante e teste de aumento gradual de carga. Ambos os testes foram conduzidos com o objetivo de comparar o desempenho entre threads tradicionais e threads virtuais, utilizando scripts automatizados para garantir tempos iguais (tempo de aquecimento, tempo para coleta de lixo, etc.). O Vegeta foi utilizado para o envio das requisições e monitoramento.

    Os scripts utilizados nos experimentos e o código do servidor, estão disponíveis no repositório do GitHub: \url{https://github.com/StephanyeCunto/TCC_Java_Thread_Benchmark}.
    
    Os testes são descritos nas subseções seguintes.


    \subsection {Teste de Carga Constante}

    Para investigar se havia diferença entre os mecanismos durante um período prolongado, foram realizados testes com carga constante. Para isso, fixou-se uma taxa de requisições baseada nos limites da máquina. Mesmo não atingindo o limite máximo da máquina servidor, o sistema operacional permitiu apenas uma quantidade limitada de requisições simultâneas.
    A taxa escolhida foi de 1000 requisições por segundo, com duração de 10 minutos. Esse valor foi definido após testes preliminares que indicaram que a máquina servidor começava a apresentar erros de conexão acima dessa taxa.
    
    O teste foram realizados da seguinte maneira:
    ...
    \begin{enumerate}
        ...
        \item Pausa de 80 segundos.
        \item Teste principal com 1000 requisições por segundo durante 600 segundos.
        \item Pausa de 600 segundos.
        \item Repetição dos passos 1 a 7 para o outro mecanismo de threads, em uma nova instância da JVM.
    \end{enumerate}

    obs.: ver com a bianca numero de execucoes e o que fazer com execucoes com erro

    \subsection {Teste de Aumento Gradual de Carga}

    O objetivo desse teste foi identificar o ponto em que o servidor começa a apresentar erros de conexão, indicando que atingiu seu limite máximo de requisições processáveis. Para isso, a carga foi aumentada gradualmente até que o servidor não conseguisse mais atender às requisições.
    ...
    \begin{enumerate}
        ...
        \begin{enumerate}
            ...
            \item Repetição dos passos acima mais duas vezes.
        \end{enumerate}
